...
class GameStore
  ...
  def list_games
    if games.empty?
      puts 'No games found in the catalog! 🙁'
    else
  ...
  def load_data
    ...
    @games = games_data.map do |game_data|
      authors = game_data['authors'].map { |author_data| Author.new(author_data) }
      Game.new(game_data['title'], game_data['multiplayer'], Date.parse(game_data['last_played_at']), Date.parse(game_data['publish_date']), authors)
    end
    authors_data.each do |author_data|
      author = Author.new(author_data)
      author.items = games.select { |game| game.authors.include?(author) }
      add_author(author)
    end
  end
end
